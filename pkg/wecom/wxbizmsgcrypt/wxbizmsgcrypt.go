...
func (c CryptError) Error() string {
	return fmt.Sprintf("CryptError: err_code=%d, err_msg=%s", c.ErrCode, c.ErrMsg)
}
...
func (self *WXBizMsgCrypt) VerifyURL(msg_signature, timestamp, nonce, echostr string) ([]byte, error) {
	...
	if nil != err {
		hlog.Info(fmt.Sprintf("111err:%+v", err))
		return nil, err
	}
